        ...
        <div className="w-full border-t-[3px] border-slate-900">
          {servicesList.map((service, idx) => (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between py-10 md:py-14 px-4 md:px-8">
                
                {/* Left Side: Number & Title */}
                <div className="flex items-center gap-8 md:gap-16 w-full md:w-auto mb-6 md:mb-0">
                  <span className={`text-2xl md:text-4xl font-black transition-colors duration-400 ${hoveredIndex === idx ? service.textColor : 'text-slate-300'}`}>
                    {service.id}
                  </span>
                  <h3 className={`text-4xl md:text-6xl lg:text-[80px] font-black uppercase tracking-tighter transition-colors duration-400 leading-none ${hoveredIndex === idx ? service.textColor : 'text-slate-950'}`}>
                    {service.title}
                  </h3>
                </div>
